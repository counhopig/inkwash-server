...
- No test CI. Pre-commit verification: `cargo fmt --check && cargo clippy --all-targets && cargo test` (10 unit tests: db CRUD/transactions in `db.rs` + auth dispatch in `routes.rs`) plus `npm run build` in `admin-ui/` (= `vue-tsc --noEmit` + vite build). The only workflow is `.github/workflows/release.yml` (below).
...
- Data SQL is written with `?` placeholders for SQLite; `Db::adapt()` rewrites them to `$1, $2, …` for Postgres at runtime, because sqlx 0.8's Postgres driver does **not** translate `?` itself. Never write `$N` placeholders directly (SQLite rejects them), and never put `?` inside a SQL string literal (the adapter would miscount).
