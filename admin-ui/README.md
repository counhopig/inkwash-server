...
- `src/App.vue` - coordinator: owns the session, device list + per-device
  stats, and the shared toast/confirm/busy plumbing (no router; views
  switch via a `view` ref).
- `src/LoginView.vue` / `src/DashboardView.vue` / `src/DeviceView.vue` /
  `src/AccountView.vue` - the layered views (sign in / create account /
  admin-token fallback; device cards + register device; alarms/todos +
  channels + inbox editor for one device; session info, change password).
- `src/lib/api.ts` - typed `fetch()` wrappers over the admin API.
- `src/lib/generated/` - ts-rs bindings regenerated by `cargo test`
  (server-side `#[derive(TS)]` DTOs; `src/lib/types.ts` re-exports them).
- `src/lib/ui.ts` - `InjectionKey` plumbing (`ui.run(name, fn)` guard,
  shared 401/error handling).
- `src/lib/clipboard.ts` - copy-to-clipboard with a fallback for insecure
  ...
  on the LAN).
- `src/lib/storage.ts` - persists the session and admin token in
  `localStorage`.
